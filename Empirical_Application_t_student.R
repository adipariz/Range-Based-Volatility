###########################################
###    Empirical Application  t-student ###
###########################################
library(rugarch)
library(Rcpp)
library(GAS)
...
# Setup
spec_garch <- ugarchspec(mean.model = list(armaOrder = c(0, 0), include.mean = FALSE), variance.model = list(model = 'sGARCH', garchOrder = c(1, 1)), distribution = 'std')
spec_gjr <- ugarchspec(mean.model = list(armaOrder = c(0, 0), include.mean = FALSE), variance.model = list(model = 'gjrGARCH', garchOrder = c(1, 1)), distribution = 'std')
...
for (stock_name in ibrx_names) {
  ...
  for (i in 1:oos) {
    ...
    parametros_rbgarch_p  <-  rbgarch_t_student(stock_window_ret, stock_window$parkinson)
    parametros_rbgarch_gk <-  rbgarch_t_student(stock_window_ret, stock_window$garman_Klass)
    parametros_rbgarch_m  <-  rbgarch_t_student(stock_window_ret, stock_window$meilijson)
    parametros_rbgarch_rs <-  rbgarch_t_student(stock_window_ret, stock_window$rogers_satchell)
    
    parametros_rbgjr_p  <-  rbgjr_t_student(stock_window_ret, stock_window$parkinson)
    parametros_rbgjr_gk <-  rbgjr_t_student(stock_window_ret, stock_window$garman_Klass)
    parametros_rbgjr_m  <-  rbgjr_t_student(stock_window_ret, stock_window$meilijson)
    parametros_rbgjr_rs <-  rbgjr_t_student(stock_window_ret, stock_window$rogers_satchell)
  ...
  write.csv(sigma2_fit, paste0("sigma2_t_", stock_name, ".csv"))
  write.csv(var_1, paste0("var_1_t_", stock_name, ".csv"))
  write.csv(var_2, paste0("var_2_t_", stock_name, ".csv"))
  write.csv(var_5, paste0("var_5_t_", stock_name, ".csv"))
  
  write.csv(es_1, paste0("es_1_t_", stock_name, ".csv"))
  write.csv(es_2, paste0("es_2_t_", stock_name, ".csv"))
  write.csv(es_5, paste0("es_5_t_", stock_name, ".csv"))
